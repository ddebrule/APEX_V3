    ...
    <div className="flex h-full bg-[#121212] text-white font-sans overflow-auto relative">
      ...
      <div className="w-[380px] bg-[#0d0d0f] border-r border-white/5 flex flex-col z-50 transition-transform">
        ...
        <div className="p-4 bg-white/[0.02] border-b border-white/10 flex justify-between items-center">
          <span className="text-sm text-[#E53935] font-black uppercase tracking-[2px]">◆ Event Config</span>
        ...
        <div className="p-[30px] flex-1 overflow-y-auto space-y-5">
          {/* Event Name */}
          <div>
            <label className="text-xs font-extrabold text-[#555] uppercase block mb-2">Event Name</label>
            <input
              type="text"
              value={eventName}
          ...
          {/* Session Type */}
          <div>
            <label className="text-xs font-extrabold text-[#555] uppercase block mb-2">Session Type</label>
            <select
              value={sessionType}
          ...
          {/* Vehicle Display */}
          <div>
            <label className="text-xs font-extrabold text-[#555] uppercase block mb-2">Vehicle</label>
            <div className="bg-black border border-white/5 p-2 text-sm text-[#999] font-mono rounded rounded">
          ...
          {/* Racer Display */}
          <div>
            <label className="text-xs font-extrabold text-[#555] uppercase block mb-2">Racer</label>
            <div className="bg-black border border-white/5 p-2 text-sm text-[#999] font-mono rounded">
              {selectedRacer?.name || '—'}
            </div>
          </div>
          ...
          <button
            ...
            className={`w-full border font-black uppercase text-xs p-2.5 rounded tracking-wider transition-all mt-5 ${
      ...
      <div className="flex-1 flex flex-col p-[25px] gap-[25px] overflow-hidden">
        ...
        <div className="bg-[#121214] border border-white/5 rounded flex flex-col overflow-hidden shrink-0">
          <div className="p-4 bg-white/[0.02] border-b border-white/5">
            <span className="text-sm text-[#E53935] font-black uppercase tracking-[2px]">◆ LiveRC Feed</span>
          </div>

          <div className="p-5 space-y-3">
            <label className="text-xs font-extrabold text-[#555] uppercase block">Event URL</label>
            <input
              type="text"
              value={url}
              onChange={(e) => validateUrl(e.target.value)}
              className={`w-full bg-black border rounded px-3 py-2 text-xs font-mono focus:outline-none transition-colors ${
                url && (urlValid ? 'border-[#2196F3]' : 'border-[#E53935]')
              } ${!url && 'border-white/5'} focus:border-[#E53935]`}
              placeholder="Paste LiveRC event URL here"
            />
            {urlError && <p className="text-[11px] text-[#E53935] mt-2 font-mono">{urlError}</p>}
            {urlValid && (
              <p className="text-[11px] text-[#2196F3] mt-2 font-mono">✓ Valid LiveRC URL</p>
            )}
          </div>
        </div>
        ...
        <div className="bg-[#121214] border border-white/5 rounded flex flex-col overflow-hidden shrink-0">
          <div className="p-4 bg-white/[0.02] border-b border-white/5">
            <span className="text-sm text-[#E53935] font-black uppercase tracking-[2px]">◆ Track Context</span>
          </div>
        ...
        {selectedVehicle && (
          <div className="bg-[#121214] border border-white/5 rounded flex flex-col overflow-hidden flex-1 min-h-0">
            <div className="p-4 bg-white/[0.02] border-b border-white/5 shrink-0">
              <span className="text-sm text-[#E53935] font-black uppercase tracking-[2px]">◆ Vehicle Setup</span>
            </div>
            <div className="p-5 overflow-auto">
