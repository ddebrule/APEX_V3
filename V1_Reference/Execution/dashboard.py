...
# Initialize database FIRST, before Streamlit captures output
import sys
import os
...
load_dotenv()
from Execution.database.database import get_or_create_default_profile
...
from Execution.utils.lazy_tabs import lazy_tab_renderer, initialize_tab_tracking
...
def init_session_state():
    ...
    if "racer_profile" not in st.session_state:
        ...
        st.session_state.racer_profile = profile_service.get_profile(profile_id)
        st.session_state.profile_id = profile_id

        # Auto-sync fleet for default profile on first load
        from Execution.components.sidebar import _sync_fleet_for_profile
        _sync_fleet_for_profile(st.session_state.racer_profile)
    ...
    if "practice_rounds_scheduled" not in st.session_state:
        st.session_state.practice_rounds_scheduled = 0

    # --- Persona System (Phase 5.1: Persona Restoration) ---
    if "scenario" not in st.session_state:
        st.session_state.scenario = "A"  # Default to aggressive (A: >=3 rounds, B: <3 rounds)
    if "change_history" not in st.session_state:
        st.session_state.change_history = []  # List of dicts: {timestamp, parameter, old_value, new_value, persona_key}
    if "driver_confidence" not in st.session_state:
        st.session_state.driver_confidence = 3  # 1-5 scale; <3 = changes rejected by Engineer
    if "experience_level" not in st.session_state:
        st.session_state.experience_level = "Intermediate"  # "Sportsman", "Intermediate", or "Pro"
...
# === 5. TAB NAVIGATION & RENDERING (Phase 6.5.1: Lazy Loading) ===

# Initialize tab tracking for lazy loading
...
# Render each tab directly (Lazy loading disabled to fix rendering issue)
with tab1:
    event_setup.render()
